...
saved_video_path = 'C:\BootCamp\project\pythonProject\\video stabilization\input2.mp4'
    ...
    while True:
        if activate_thread:
            if video_data:
                ...
                stabilized_frames = stabilization.stabilize_video_sift_optical_flow(saved_video_path)
...
class ImageProcessingServicer(stereo_vehicle_pb2_grpc.ImageProcessingServicer):
    ...
    def FindHeight(self, request, context):
        ...
        height = find_height.calculate_height_above_ground(x, y, left_image, right_image, focal_length, baseline,
                                                           camera_height)
    ...
    def IdentifyVehicle(self, request, context):
        ...
        results = detect_new.process_video(video_path, img_path, output_folder)
...
#######################
    def GetStabilizedFrame(self, request, context):
        frame = stabilization.get_stabilized_frame()
